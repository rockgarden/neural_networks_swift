...
public extension NeuralNet {
    ...
    public enum ActivationFunction {
        ...
        /// MARK: Hidden Activation Functions 隐藏激活函数
        ...
        /// The derivative accepts a vector of each node's OUTPUT, and computes the gradient with respect to the node's INPUT.派生函数接受每个节点的OUTPUT的矢量，并计算相对于该节点的INPUT的梯度。
        /// Note that no error/cost function is applied here.
        /// The derivative is used by `NeuralNet` to propagate the error from the succeeding layer.使用导数来传播来自后一层的错误。
